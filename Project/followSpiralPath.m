function followSpiralPath()
filename = 'SpiralPath.gif';
RECORD = true;
frames = [];
% close(figure(1));
f = figure(1);
...
dt_highlevel = 0.1;
dt_video = 1;
A = arm([l1, l2, l3, l4, l5, l1, l2, l3, l4, l5]);
A = A.set_joints(pi/4 * ones(1, A.num_links));
A = A.set_joint_vel(zeros(1, A.num_links));
A = A.calc_poses();
A = A.calc_vels();
start_pt = A.links(end).distal(1:3);
travel_path = SpiralPath([1,1,1], linspace(0,16*pi,points), -0.5, 0.1);
A.drawArm(ax);
i = 1;
i_old = 0;
i_frame = 0;
